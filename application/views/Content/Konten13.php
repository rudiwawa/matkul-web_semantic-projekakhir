...
<section id="produk">
    ...
    <div class="col-12 card shadow mt-5 mb-5">
        <h4 class="my-3">Data Notifikasi</h4>
                    ...
                    <form method="POST" id="form" class="form-horizontal need-validate" novalidate>
                        <div class="box-body">
                            <div class="row clearfix">
                                <div class="col-md-12">
                                    <label for="jenis_konten" class="control-label">Jenis Konten</label>
                                    <div class="form-group">
                                        <select type="text" name="jenis_konten" value="" class="form-control" id="jenis_konten"
                                                required '/>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                    <input type="text" name="judul" value="" class="form-control" id="judul"
                                               required/>
                                    </div>
                                </div>
                                <div class="col-md-12">
...
<script>
    let m_view = 'view';
    let bash_api = "<?php echo base_url('sapi/notifikasi/') ?>";
    console.log(bash_api);
    var number, is_update;
    let label = " Notifikasi ";
    ...
    var table = $('#table').DataTable({
        ...
        "columns": [{
            ...
            "render": function (data, type, JsonResultRow, meta) {
                return '<img src="' + "<?php echo base_url('') ?>" + "/uploads/notifikasi/" + JsonResultRow.foto + '" alt="..." class="img-thumbnail style="width:200px;">'
            }
        }, {
            "render": function (data, type, JsonResultRow, meta) {
    ...
    function read(ID, state = "read") {
        ...
        $.ajax({
            ...
            success: function (r) {
                ...
                if (r.error == false) {
                    conf_state(state);
                    $("select[id='jenis_konten'] option[value=" + r.data.jenis_konten + "]").attr("selected", "selected");
                    // $('#jenis_id').val(r.data.jenis_id);
                    // $('#kategori_id').val(r.data.kategori_id);
                    $('#jenis_konten').val(r.data.jenis_konten);
                    $('#detail').val(r.data.detail);
                    $('#judul').val(r.data.judul);
                    // $('#foto').val(r.data.foto);
                    $("#image_preview").append('<div class="show-image"><img src="' + "<?php echo base_url('') ?>" + "/uploads/notifikasi/" + r.data.foto + '" class="rounded image_view p-1" alt="..." style="width:100%;" id="img_preview_src">');
